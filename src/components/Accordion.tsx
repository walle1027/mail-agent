...
const AccordionItem: React.FC<AccordionItemProps> = ({ title, content, isOpen, toggleOpen }) => {
  return (
    <div className="mb-4">
      <div className={`w-full rounded-lg overflow-hidden ${isOpen ? 'bg-black dark:bg-gray-900' : 'bg-black dark:bg-gray-900'}`}>
...
const defaultAccordionItems = [
  ...
  {
    title: 'Easy to use',
    content: 'Our app is designed to be user-friendly and easy to use. We want to provide a seamless experience for your users so that you can focus on what matters.',
  },
  {
    title: 'Focus on what matters',
    content: 'With better financial management, you can reduce money-related stress and focus on what matters most. More to come.',
  },
]
  ...
  return (
    <div className="w-[90%]">
      {items.map((item, index) => (
        <AccordionItem key={index} title={item.title} content={item.content} isOpen={openIndex === index} toggleOpen={() => toggleItem(index)} />
      ))}
    </div>
  )
}
